...
for i in range(np.shape(X_df)[1]):
    ...
    X_df[tag_name] = dpm.standardise(X_df.iloc[:, i], X_mean, X_std)
    
    # Re-write X_df_test now with standardise data (at training points)
    X_df_test[tag_name] = dpm.standardise(X_df_test.iloc[:, i], X_mean, X_std)
...
# Save memory
# Y_df, Y_df_test,
del X_df, X_df_test, T_df, Y_raw_df, Y_raw_df_test, dpm

# The training data
X_train = X[start: end0]
Y_train = Y_raw_stand[start: end0]
Y_filt = Y[start:end0]
dt_train = date_time[start:end0]
N_train = len(Y_train)
...
ax.plot(dt_test, mu - Y_raw_mean, color="red", linewidth = 2.5, label="DP-GP")
# plt.axvline(date_time[9600], linestyle='--', linewidth=3, color='lime')
...
fig.autofmt_xdate()
for k in range(Ngps):
    ax.plot(dt_test, betas[:,k], color=c[k], linewidth=2,
            label='Beta: '+str(k))
    print('step: ', int(k*step))
    plt.axvline(dt_test[int(k*step)], linestyle='--', linewidth=2,
                color='black')
    
plt.axvline(dt_test[int((k + 1)*step)], linestyle='--', linewidth=3,
            color='lime')
