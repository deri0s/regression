...
main_df = pd.read_excel('Timelag_samples.xlsx', sheet_name='time_lags')
"""
...
T_df = pd.read_excel('Timelag_samples.xlsx')
...
#-----------------------------------------------------------------------------
# INITIALISE WORKSHEETS FOR EACH TIME LAG SAMPLE SIMULATION
#-----------------------------------------------------------------------------
    
worksheet = workbook.add_worksheet('Results.xlsx')
...
worksheet.write(0, len(T_df.columns), 'Error', font_title)
...
for i in range(N_sim):
    ...
    # Get the sampled timelags
    timelags = T_df.iloc[i,:]
    
    # Process training data
    X, Y, N, D, max_lag, time_lags = dpm.align_timelags(X_df,
                                                        Y_df,
                                                        timelags)

    # Process testing data
    X_test, Y_test, N_test, D, max_lag, time_lags = dpm.align_timelags(X_df_test,
                                                                       Y_df_test,
                                                                       timelags)
