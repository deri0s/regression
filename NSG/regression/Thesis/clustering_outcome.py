...
# Save memory
del X_df, T_df, Y_df, Y_raw_df, dpm
...
fig.autofmt_xdate()
ax.fill_between(dt_train, mu[:,0] + 3*std[:,0], mu[:,0] - 3*std[:,0],
                alpha=0.5, color='pink',
                label='Confidence \nBounds (DP-GP)')
ax.plot(dt_train, mu, color="red", linewidth = 2.5, label="DPGP")
...
ax.plot(dt_train, mu, color="green", linewidth = 2, label=" DPGP expert")
ax.set_xlabel(" Date-time", fontsize=14)
ax.set_ylabel(" Fault density", fontsize=14)
plt.legend(loc=0, prop={"size":18}, facecolor="white", framealpha=1.0)
