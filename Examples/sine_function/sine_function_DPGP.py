...
# Create and train the robust GP that uses the EM algoruthm for inference
mixGP = EMGP(X, Y, init_K=3, kernel=kernel, normalise_y=True, N_iter=10,
             plot_conv=True)
mixGP.train()
muMix, stdMix = mixGP.predict(xNew, return_std=True)

# DPGP
dpgp = DPGP(X, Y, init_K=7, kernel=kernel, normalise_y=True, plot_conv=True)
dpgp.train()
mu, std = dpgp.predict(xNew)
...
plt.plot(xNew, muMix, color='orange', linestyle=':', linewidth = 4,
         label='EM-GP')
plt.plot(xNew, mu, color='green', linestyle=':', linewidth = 4,
         label='DPGP')
...
plt.plot(xNew, mu, linewidth=2.5, color='green', label='EM-GP')
plt.xlabel('x', fontsize=16)
plt.ylabel('f(x)', fontsize=16)
plt.legend(prop={"size":20})

# ----------------------------------------------------------------------------
# MODEL CONVERGENCE COMPARISONS
# ----------------------------------------------------------------------------

plotc = mixGP.convergence
plt.figure()
plt.plot(plotc, '-o', color='green')
