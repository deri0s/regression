...
"""
ROBUST GAUSSIAN PROCESS REGRESSION USING A MIXTURE OF GAUSSIAN DISTRIBUTIONS
NOISE MODEL

This algorihm uses a K-component Gaussian mixture model to form the likelihood.
The aim is to identify 'outlier' data points in the analysis
and train a GP on the remaining 'optimum' data points that are left over.
This algorithm uses the GMM class of the SKLearn repository

Diego Echeverria Rios & P.L.Green
"""


class ExpectationMaximisationGaussianProcess(GPR):
    """In the present analysis the noise corrupting the data is assumed to be
       generated from a mixture of K Gaussian distributions. We define
       noise_sources = K
    """

    def __init__(self, X, Y, init_K, kernel=RBF,
                 normalise_y=False, N_iter=5, plot_conv=False, plot_sol=False):
        ...
        # mu always vstack and not the errors vector
        mu = np.vstack(super().predict(self.X, return_cov=False))
        
        # Initialise the residuals and initial GP hyperparameters
        self.init_errors = self.Y - mu
        self.hyperparameters = self.kernel_.theta
        print('The very first hyper: ', self.kernel_,'\n')

        # Initialise axis and labels if plot solution is required
    ...
    def clustering(self, X, Y, errors):
        """
        ...
        gmm =m.GaussianMixture(n_components=self.init_K,
        ...
        return indices, X0, Y0, resp[0], pies, stds
    ...
    def train(self):
        ...
        # The log-likelihood with the initial hyperparameters
        lnP[i] = self.log_marginal_likelihood()
        
        # Stop if the change in the log-likelihood is no > than 10% of the 
        # log-likelihood evaluated with the initial hyperparameters
        tolerance = abs(lnP[0]*12)/100
        
        while i < max_iter:
            # The clustering step
            index, X0, Y0, resp0, pies, stds = self.clustering(self.X, self.Y,
                                                               errors)
            # In case we want to know the init mixture parameters
            if i == 1:
        ...
        if self.plot_conv:
            self.plot_convergence(lnP, 'EMGP convergence')
